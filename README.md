...
이 프로젝트는 **Spring Boot(근태 관리)**와 **MySQL `smart_factory` DB를 공유**합니다.

### 테이블 분리 전략
- **Flask 전용**: car, sensor_result, camera_result, defect_image
- **Spring 전용**: attendance, attendance_log
- **공유**: employee (양쪽에서 읽기만 수행)

### 데이터 격리
- 각 서버는 자신의 도메인 테이블만 수정
- Cross-domain write 금지로 충돌 방지
