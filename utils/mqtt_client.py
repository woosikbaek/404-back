...
def on_connect(client, userdata, flags, rc):
    ...
    client.subscribe(TOPIC_SENSOR_CONTROL)
    print(f"[MQTT] 구독 토픽: {TOPIC_SENSOR_CONTROL}, {TOPIC_SENSOR_RESULT}, {TOPIC_CAMERA01_RESULT}")
...
def on_message(client, userdata, msg):
    with _flask_app.app_context():
        try:
            ...
            if msg.topic == TOPIC_SENSOR_CONTROL:
                data = msg.payload.decode()
                print(f"[sensor/control] 수신: {data}")
